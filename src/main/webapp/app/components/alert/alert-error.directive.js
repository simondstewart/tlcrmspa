(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('tlcrmspaApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-tlcrmspaApp-error');
                var entityKey = httpResponse.headers('X-tlcrmspaApp-params');
                if (errorHeader) {
